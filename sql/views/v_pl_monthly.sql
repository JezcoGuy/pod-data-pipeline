...
CREATE OR REPLACE VIEW v_pl_monthly AS
 WITH sales AS (
         ...
         SELECT date_trunc('month'::text, (orders.refunded_at AT TIME ZONE 'Europe/London'::text))::date AS month,
            sum(COALESCE(orders.refund_amount_gbp, 0::numeric)) AS refunds
           FROM orders
          WHERE orders.brand_id::text = 'your_brand_id'::text AND (orders.financial_status::text = ANY (ARRAY['refunded'::character varying, 'partially_refunded'::character varying]::text[])) AND orders.refunded_at IS NOT NULL
          GROUP BY (date_trunc('month'::text, (orders.refunded_at AT TIME ZONE 'Europe/London'::text))::date)
        ), cogs AS (
        ...
        UNION
         SELECT amex.month
           FROM amex
        UNION
         SELECT meta_spend_cte.month
           FROM meta_spend_cte
        UNION
         SELECT monzo.month
           FROM monzo
        UNION
         SELECT payouts.month
           FROM payouts
        ), base AS (
         SELECT m.month,
