-- Migration v8.23.1 — expose raw due_date on v_priority_tasks
-- ------------------------------------------------------------
-- The task manager HTML/API needs the raw due_date (ISO YYYY-MM-DD) to
-- populate the Reschedule inline date picker. The labelled "Due" column
-- (e.g. "🟡 Due in 3 days") is for display only and can't be parsed back
-- into a date input. Adding due_date alongside; "Due" stays unchanged for
-- existing consumers.
...
-- Column order preserves the original public surface; due_date is appended
-- so CREATE OR REPLACE doesn't trip Postgres' "can't rename column" guard.
SELECT
    ...
    notes,
    due_date                           -- ← NEW: raw ISO date for the inline picker
FROM scored
ORDER BY score DESC;
